calcDelta <- function(yr, spec.abund){
    ...
    sp.yr1 <- unique(spec.abund$GenusSpecies[spec.abund$Year == yr & #changed from GenusSpecies (bee) to PlantGenusSpecies
...
calcPlantDelta <- function(yr, plant.abund){
  print(yr)
  ## sites sampled both years
  sites.yr1 <- unique(plant.abund$Site[plant.abund$Year == yr])
  sites.yr2 <- unique(plant.abund$Site[plant.abund$Year == yr + 1])
  sites.both.yrs <- sites.yr1[sites.yr1 %in% sites.yr2]
  ...
  ## create a community matrix of species abundances by site for yr1
  ## and yr2 to ensure alignment for log ratio
  abund.yr.one <- plant.abund[plant.abund$Year == yr,]
  abund.yr.one <- samp2site.spp(abund.yr.one$Site,
                                abund.yr.one$PlantGenusSpecies,
                                abund.yr.one$PlantAbund)
  
  abund.yr.two <- plant.abund[plant.abund$Year == (yr + 1),]
  abund.yr.two <- samp2site.spp(abund.yr.two$Site,
                                abund.yr.two$PlantGenusSpecies,
                                abund.yr.two$PlantAbund)
...
calcYearDiff <- function(spec.abund){
    ## function for calculating the log ratio of pollinator abundance (changed to plant abundancea)
    ## between any arbitrary number of years
    years <- as.numeric(as.character(unique(spec.abund$Year)))
    years <- years[-length(years)]
    deltas <- lapply(years, calcSpecDelta, spec.abund=spec.abund)
    names(deltas) <- years
    return(deltas)
}

calcPlantYearDiff <- function(plant.abund){
  ## function for calculating the log ratio of pollinator abundance (changed to plant abundancea)
  ## between any arbitrary number of years
  years <- as.numeric(as.character(unique(plant.abund$Year)))
  years <- years[-length(years)]
  deltas <- lapply(years, calcPlantDelta, plant.abund=plant.abund)
  names(deltas) <- years
  return(deltas)
}
...
prepDeltaVar <- function(delta, spec.abund, site.char, veg){
    ...
    spec.abund <- spec.abund[spec.abund$Year == "2019",] #Changed to 2019
    ...
    # site.data.2019 <- site.data[site.data$Year == "2019",] #changed from 2013 to 2019
    # site.data.2020 <- site.data[site.data$Year == "2020",] #changed from 2024 to 2020
    # ## calculate the change in floral richness
    # site.data.2019$deltaFloralRichness <-
    #     log(site.data.2020$FloralRichness)/log(site.data.2019$FloralRichness)
    # delta.site.sp.char <- merge(delta.sp.char, site.data.2019)
    ...
    veg.2019 <- veg.ave[veg.ave$Year == "2019",]
    veg.2020 <- veg.ave[veg.ave$Year == "2020",]
    ## calculate the change in floral richness
    veg.2019$deltaFloralAbund <-
        veg.2020$logFlowerNum/veg.2019$logFlowerNum
    delta.site.sp.char <- merge(delta.site.sp.char, veg.2019)
    ...
    return(list(delta=delta.site.sp.char, site.data= site.data.2019))
